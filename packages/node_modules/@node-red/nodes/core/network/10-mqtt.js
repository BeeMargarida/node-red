...
module.exports = function (RED) {
    ...
    function MQTTInNode(n) {
        ...
        this.setGenerateCodeFunction(() => generateMicropythonCode(this.topic));
        
        n.inputTopics.push(this.topic);
        ...
        generateMicropythonCode(this.topic);
         ...
         * @param {*} rules 
         */
        function generateMicropythonCode(topic) {
            const code =
                `\nimport ubinascii
gc.collect()
input_topics_${node.textId} = ["${topic}"]
output_topics_${node.textId} = [${node.outputTopics.map(a => `"${a}"`)}]
node_datatype_${node.textId} = "${node.datatype}"
node_qos_${node.textId} = ${node.qos}

def on_input_${node.textId}(topic, msg, retained):
    if node_datatype_${node.textId} == "base64":
        msg = str(ubinascii.b2a_base64(msg))
    elif node_datatype_${node.textId} == "utf8":
        msg = msg.encode("utf-8")
    elif node_datatype_${node.textId} == "json":
        try:
    ...
    msg = dict(
        topic=topic,
        payload=msg,
        qos=node_qos_${node.textId},
        retain=retained
    )
    loop = asyncio.get_event_loop()
    loop.create_task(on_output(ujson.dumps(msg), output_topics_${node.textId}))\n`
...
output_topics_${node.textId} = ["${topic}"]
