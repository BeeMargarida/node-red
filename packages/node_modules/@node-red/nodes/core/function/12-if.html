...
<script type="text/html" data-template-name="if">
    ...
    <div class="form-row" style="margin-bottom: 0px">
        <label for="node-input-tags"><span data-i18n="if.label.tags"></span></label>
        <div style="display: inline-block; width: calc(100% - 105px)"><input type="text" id="node-input-tags"></div>
    </div>
    <div class="form-row">
        <label for="node-input-property"><i class="fa fa-ellipsis-h"></i> <span data-i18n="if.label.property"></span></label>
    ...
    <div class="form-row">
        ...
        <label style="width: auto;" for="node-input-repair"><span data-i18n="if.label.repair"></span></label></input>
    </div>
</script>
...
<script type="text/javascript">
    (function () {
        var operators = [
            ...
            { v: "gt", t: ">", kind: 'V' },
            { v: "gte", t: ">=", kind: 'V' },
            { v: "btwn", t: "if.rules.btwn", kind: 'V' },
            { v: "true", t: "if.rules.true", kind: 'V' },
            { v: "false", t: "if.rules.false", kind: 'V' },
            { v: "null", t: "if.rules.null", kind: 'V' },
            { v: "nnull", t: "if.rules.nnull", kind: 'V' },
        ];
        ...
        RED.nodes.registerType('if', {
            ...
            oneditprepare: function () {
                var node = this;
                var previousValueType = { value: "prev", label: this._("if.previous"), hasValue: false };
                    ...
                    addItem: function (container, i, opt) {
                        ...
                        for (var d in operators) {
                            if (operators[d].kind === 'V') {
                                group0.append($("<option></option>").val(operators[d].v).text(/^if/.test(operators[d].t) ? node._(operators[d].t) : operators[d].t));
                            }
                        }
                        var group1 = $('<optgroup/>', { label: "sequence rules" }).appendTo(selectField);
                        for (var d in operators) {
                            if (operators[d].kind === 'S') {
                                group1.append($("<option></option>").val(operators[d].v).text(/^if/.test(operators[d].t) ? node._(operators[d].t) : operators[d].t));
                            }
                        }
                        for (var d in operators) {
                            if (operators[d].kind === 'O') {
                                selectField.append($("<option></option>").val(operators[d].v).text(/^if/.test(operators[d].t) ? node._(operators[d].t) : operators[d].t));
                            }
                        }
