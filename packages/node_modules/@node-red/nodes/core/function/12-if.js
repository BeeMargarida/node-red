    ...
    function IfNode(n) {
        ...
        function generateMicropythonCode(rules) {
            ...
            for (let i = 0; i < rules.length; i++) {
                ...
                const ifRules = `def if_rule_${i}(a${pythonParameters(valuesRule)}):\n    ${typeConverter(rules[i].vt)}\n    return ${pythonOperators[rules[i].t]}\n`;

                rulesBlock += i == 0 ? `if_rule_${i}(a)` : ` and if_rule_${i}(a)`;
                ifBlock += ifRules;
            }


            ifBlock += `def if_function(a):\n    res = ${rulesBlock}\n    return '%s' % res`;
...
def on_input(topic, msg, retained):
    ...
    loop.create_task(on_output(res))
...
async def on_output(msg):
    for output_topic in output_topics:
        await mqtt_client.publish(output_topic, msg)
...
def exec(mqtt_c):
    ...
    return`
            ...
            const fs = require('fs');
            fs.writeFile(`python-scripts/12-if-${node.id}.py`, code, function (err) {
                if (err) {
                    return console.log(err);
                }
                console.log("The file was saved!");
