...
module.exports = function (RED) {
    ...
    function IfNode(n) {
        ...
        this.setGenerateCodeFunction(() => generateMicropythonCode(this.property, this.rules));

        this.setTags(["micropython"])
         ...
         * [MINE]
         * 
         * Generates micropython code that executes the if behaviour
         * 
         * @param {*} rules 
         */
        function generateMicropythonCode(property, rules) {
            console.log("MICROPYTHON CODE")
            let code = "";
            ...
            for (let i = 0; i < rules.length; i++) {
                ...
                const ifRules = `def if_rule_${node.textId}_${i}(a${pythonParameters(valuesRule)}):\n    ${typeConverter(rules[i].vt)}\n    return ${pythonOperators[rules[i].t]}\n`;

                rulesBlock += i == 0 ? `if_rule_${node.textId}_${i}(a)` : ` and if_rule_${node.textId}_${i}(a)`;
                ifBlock += ifRules;
            }


            ifBlock += `def if_function_${node.textId}(a):\n    res = ${rulesBlock}\n    return '%s' % res`;

            code =
                `\ninput_topics_${node.textId} = ["${inputTopic}"]
output_topics_${node.textId} = [${node.outputTopics.map(a => `"${a}"`)}]
property_${node.textId} = "${property}"

${ifBlock}

def get_property_value_${node.textId}(msg):
    properties = property_${node.textId}.split(".")
    ...
    for property in properties:
        try:
            if payload[property]:
                ...
                print("${node.textId}: Property not found")
                break
        except:
            print("${node.textId}: Msg is not an object")
            break

    return payload

def on_input_${node.textId}(topic, msg, retained):
    msg = get_property_value_${node.textId}(msg)
    res = if_function_${node.textId}(msg)

    # Create task to publish to output topics
    loop = asyncio.get_event_loop()
    loop.create_task(on_output(res, output_topics_${node.textId}))\n`
