...
module.exports = function (RED) {
    ...
    function IfNode(n) {
        ...
        // MICROPYTHON CODE
        generateMicropythonCode(this.rules);

        // const fs = require('fs');
        // fs.writeFile(`python-scripts/12-if-${node.id}.py`, node.micropythonCode, function (err) {
        //     if (err) {
...
def on_input_${node.textId}(topic, msg, retained):
    ...
    loop.create_task(on_output(res, output_topics_${node.textId}))\n`
            ...
            const fs = require('fs');
            fs.writeFile(`python-scripts/12-if-${node.textId}.py`, code, function (err) {
                if (err) {
                    return console.log(err);
                }
                console.log("The file was saved!");
