...
module.exports = function (RED) {
    ...
    function IfNode(n) {
        ...
        function pythonParameters(operator, values) {
            let noValue = ['true', 'false', 'null', 'nnull'];
            let oneValue = ['eq', 'neq', 'lt', 'lte', 'gt', 'gte'];
            let twoValues = ['btwn'];
            if (noValue.includes(operator)) return "";
            if (oneValue.includes(operator) && values.length == 1) return `,b = ${values[0]}`;
            if (twoValues.includes(operator) && values.length == 2) return `, b = ${values[0]}, c = ${values[1]}`
        }
        ...
        function generateMicropythonCode(rules) {
            // rules: { t -> operator, v -> value to compare to, vt -> type of value}
            ...
            const values = Object.keys(rules).filter(k => {
                return k.indexOf('v') === 0 && k.length == 1
            }).map((k) => (rules[k]))

            // TODO: support multiple inputs and outputs
            const inputTopic = node.inputTopics[0];
            const outputTopic = node.outputTopics[0];
            ...
            for (let i = 0; i < rules.length; i++) {
                ...
                const values = Object.keys(rules[i]).filter(k => {
                    return k.indexOf('v') === 0 && k.length == 1
                }).map((k) => (rules[k]));
                
                const switchRules = `
                    def switchRule${i}(a, ${pythonParameters(rules[i].t, values)}):
                        return ${pythonOperators[rules[i].t]}
                `;

                rulesBlock += i == 0 ? `switchRule${i}` : ` && switchRule${i}`;
                switchBlock += switchRules;
            }


            switchBlock += `
                def switch(a${pythonParameters(rules.t, values)}):
                    return ${rulesBlock}
            `;
