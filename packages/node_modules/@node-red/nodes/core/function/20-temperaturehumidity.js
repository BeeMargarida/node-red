...
def stop_${node.textId}():
    ...
    if timer_task_${node.textId}:
        timer_task_${node.textId}.cancel()
    for timer in reference_timer_workaround:
        timer.deinit()

async def timer_exec_${node.textId}(callback, interval):
    global timer_task_${node.textId}
    if stop_repeat_${node.textId}:
        return
    callback(None)
    await asyncio.sleep_ms(interval)
    loop = asyncio.get_event_loop()
    timer_task_${node.textId} = loop.create_task(timer_exec_${node.textId}(callback, interval))
