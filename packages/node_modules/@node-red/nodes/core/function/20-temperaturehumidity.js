...
def measure(_):
    d = dht.DHT22(machine.Pin(pin_${node.textId}))
    d.measure()
    ...
    loop.create_task(on_output(encoded, output_topics_${node.textId}))
...
def exec_${node.textId}():
    if repeat_${node.textId}:
        ...
        timer.init(period=interval_${node.textId}, mode=machine.Timer.PERIODIC, callback=measure)
