    ...
    function OrchestratorNode(n) {
        ...
        this.on("input", async function (msg, done) {
            try {
                ...
                if (equalDevices(newDevices)) return;
...
async def exec(mqtt_c, capabilities_array):
    global mqtt_client
    global capabilities
    mqtt_client = mqtt_c
    ...
    return\n`
         ...
         */
        async function sendCode(code, address, nrNodes) {
            ...
            return axios({
                ...
                node.log(`SUCCESS in sending code of ${nrNodes} to device ${address}: Status ${res.status}`);
                ...
                node.log(`ERROR in sending code of ${nrNodes} to device ${address}: Status ${status}`);
                // Send telemetry to orchestrator nodes
                node.receive({ type: "telemetry", device: address, data: { duration: Date.now() - startTime, state: 0 } });
                return { id: address, nodes: nrNodes, status: status }
            })
        }
        ...
        async function distributeFlow(nodes, changes) {
            ...
            Object.entries(nodes).forEach(async ([k, flowNode]) => {
                ...
                if (flowNode.type.includes("subflow") ||
                    (!flowNode.noWiresType && flowNode.inputTopics.length === 0 && flowNode.outputTopics.length === 0)) return;
            ...
            if (Object.keys(nodeAssignment).length > 0) {
                ...
                fs.writeFile(`python-scripts/node-assignment-${Date.now()}.json`,
                    ...
                    function (err) {
                        ...
                        node.log("The node assignment file was saved!");
                    });
        ...
        async function ping() {
            ...
            for (const id in devices) {
                ...
                const promise = axios({
                    ...
                    node.log(`DEVICE ${id} STATUS: ` + res.data.status);
                    if (devices[id].status !== 1) {
                        devices[id].status = 1;
                        changes[id] = { status: 1 }
                    }
                }).catch(err => {
                    node.log(`DEVICE ${id} ERROR`);
