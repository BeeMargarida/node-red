    ...
    function OrchestratorNode(n) {
        ...
        this.on("input", function (msg, done) {
            try {
                ...
                this.devices = value;
                console.log(node.context().keys());
                console.log(node.context().global.keys());
    ...
    return\n`
            ...
            return axios({
                ...
                for (const id in this.context.nodes) {
                    if (this.context.nodes[id].type === "orchestrator") {
                        this.context.nodes[id].receive({ type: "telemetry", device: address, data: { duration: Date.now() - startTime, state: 1 } });
                    }
                }
                ...
                for (const id in this.context.nodes) {
                    if (this.context.nodes[id].type === "orchestrator") {
                        this.context.nodes[id].receive({ type: "telemetry", device: address, data: { duration: Date.now() - startTime, state: 0 } });
                    }
                }
        ...
        async function distributeFlow(nodes, changes) {
            ...
            Object.entries(nodes).forEach(([k, node], index) => {
                ...
                const deviceId = getBestDevice(this.context.devices, node, nodeAssignment);
            ...
            if (Object.keys(nodeAssignment).length > 0) {
                ...
                for (const id in this.context.nodes) {
                    if (this.context.nodes[id].type === "orchestrator") {
                        this.context.nodes[id].receive({ type: "assignment", data: nodeAssignment });
                    }
                }
            }
        ...
        async function ping() {
            ...
            for (const id in devices) {
                ...
                const promise = axios({
                    ...
                    if (devices[id].status !== 1) {
                        this.context.devices[id].status = 1;
                        changes[id] = { status: 1 }
                    }
                }).catch(err => {
                    console.log(`DEVICE ${id} ERROR: ` + err);
                    if (devices[id].status !== 0) {
                        this.context.devices[id].status = 0;
            ...
            if (Object.keys(changes).length > 0) {
                distributeFlow(this.context.nodes, changes)
            }
        }
        ...
        function setupPing() {
            ...
            this.context.pingInterval = setInterval(() => ping(), 10000)
