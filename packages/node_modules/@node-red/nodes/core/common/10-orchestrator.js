        ...
        this.on("input", async function (msg, done) {
            try {
                ...
                if (devices[value.address]) {
                    ...
                    if (value.failure === 1) {
                        const changes = {};
                        changes[value.address] = {
                            failure: 1,
                            memoryErrorNodes: nodeAssignment[value.address].nodes.length,
                            nodes: []
                        };
    ...
    return\n`
        ...
        function getBestDevice(availableDevices, flowNode) {
            ...
            for (const id in availableDevices) {
                ...
                if (nodeAssignment[id] && (nodeAssignment[id].nodes.length + 1) >= device.memoryErrorNodes) continue;
