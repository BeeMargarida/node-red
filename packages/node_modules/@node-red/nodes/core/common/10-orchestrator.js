...
module.exports = function (RED) {
    ...
    function OrchestratorNode(n) {
        ...
        console.log("Node Assignment", nodeAssignment);
        let devices = {};
        ...
        this.on("input", async function (msg, done) {
            try {
                ...
                delete value._msgid
                if (equalDevices(value)) return;

                const nodes = node._flow.activeNodes;

                if (device[value.address]) {
                    // update existing device with new failure
                    // and capabilities information
                    devices[value.address] = {
                        capabilities: value.capabilities,
                        failure: value.failure,
                        status: devices[value.address].status,
                        memoryErrorNodes: devices[value.address].memoryErrorNodes,
                        nodes: devices[value.address].nodes
                    };
                    ...
                    if (value.failure === 1) {
                        const changes = {};
                        changes[value.address] = { failure: 1, memoryErrorNodes: devices[value.address].nodes.length };
                        await distributeFlow(nodes, changes);
                    }
                }
                else {
                    // save new device and reorchestrate the flow
                    devices[value.address] = {
                        capabilities: value.capabilities,
                        failure: value.failure,
                        status: value.status,
                        memoryErrorNodes: value.memoryErrorNodes,
                        nodes: devices[id] ? devices[id].nodes : value[id].nodes
                    };
                    await distributeFlow(nodes, null);
                }
                setupPing();
            } catch (err) {
        ...
        function equalDevices(newDevices) {
            ...
            for (const id in devices) {
                if (newDevices[id].status !== devices[id].status) return false;
                if (newDevices[id].failure !== devices[id].failure) return false;
                if (newDevices[id].capabilities.sort().join(",") !== devices[id].capabilities.sort().join(",")) return false;
            }
    ...
    return\n`
        ...
        function getBestDevice(availableDevices, flowNode) {
            ...
            for (const id in availableDevices) {
                ...
                if (nodeAssignment[id] && nodeAssignment[id].nodes.filter(n => n.id === flowNode.id).length > 0 && !device.memoryErrorNodes) return id;
                ...
                if (nodeAssignment[id] && nodeAssignment[id].nodes.filter(n => n.id === flowNode.id).length > 0 &&
        ...
        async function distributeFlow(nodes, changes) {
            ...
            Object.entries(nodes).forEach(async ([k, flowNode]) => {
                console.log(flowNode.type);
                if (flowNode.type === "mqtt-broker" || flowNode.type === "tab" || flowNode.type === "registry") return;
                if (flowNode.id === node.id) return;
                if (flowNode.type.includes("subflow") || (flowNode.inputTopics.length === 0 && flowNode.outputTopics.length === 0)) return;

                // If node does not have a code generator, it only runs in node-red
                if (!flowNode.generateCode) {
                    if (nodeAssignment["node-red"] && nodeAssignment["node-red"].nodes.filter(n => n.id === flowNode.id).length === 0) {
                        nodeAssignment["node-red"].nodes.push(flowNode.info());
                    } else {
                ...
                if (deviceId) {
                    flowNode.setDevice(devices[deviceId]);
                    ...
                    if (nodeAssignment[deviceId] && nodeAssignment[deviceId].nodes.filter(n => n.id === flowNode.id).length === 0) {
                        nodeAssignment[deviceId].nodes.push(flowNode.info());
                    } else {
                    ...
                    if (flowNode.predicates.length > 0 && !flowNode.predicates.includes("node-red")) {
                        node.error(`Predicates in node ${node.id} cannot be satisfied.`);
            ...
            const assignmentResult = await generateAndSend();
            ...
            if (Object.keys(nodeAssignment).length > 0) {
                ...
                node.receive({ type: "assignment", data: nodeAssignment });
            }

            node.context().global.set("nodeAssignment", nodeAssignment, (err) => node.error(err));
            assigning = false;
            return;
        }
        ...
        function setupPing() {
            ...
            if (pingInterval !== null) return pingInterval;
