        ...
        this.on("input", async function (msg, done) {
            try {
                ...
                for (const id in newDevices) {
                    ...
                    if (devices[id] && !newDevices[id].new) newDevices[id].status = devices[id].status;
...
async def exec(mqtt_c, capabilities_array, c_id):
    ...
    return\n`
        ...
        async function ping() {
            ...
            const allDevicesDown = Object.values(changes).filter(value => value.status === 0).length === Object.keys(devices).length;

            if (Object.keys(changes).length > 0 && !allDevicesDown) {
