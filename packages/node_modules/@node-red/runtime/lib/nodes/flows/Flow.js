...
class Flow {
    ...
    constructor(parent, globalFlow, flow) {
        ...
        this.context.nodeAssignment = {};

        // @TODO: get this information from the network
        ...
        if (!this.isGlobalFlow) this.setupHeartbeat();
    }
...
async def exec(mqtt_c):
     ...
     */
    async generateAndSend(devices) {
        const promises = [];
        // For each device, generate and send code of the assigned nodes
        for (const id in devices) {
            if (devices[id].nodes.length === 0) continue;

            const code = this.generateCode(devices[id].nodes);
            promises.push(this.sendCode(code, id, devices[id].nodes.length));
        ...
        for (const val of values) {
            // checks if there was any memory problem or others and 
            // saves it in changes
            if (val.status === 413) {
                changes[val.id] = {
    ...
    getBestDevice(devices, node, nodeAssignment) {
        ...
        for (const id in devices) {
            ...
            if (node.type === "temperature-humidity" &&
                device.nodes.filter(n => n.type === "temperature-humidity").length > 0) continue;

            // keep the node assigned if node was already previously assigned to the it
            if (nodeAssignment[id] && node in nodeAssignment[id].nodes && !device.memoryErrorNodes) return id;

            // ingores if the number of nodes assigned to the device with a new one is
            // equal or greater than a previous assignment that resulted in memory error
            if (nodeAssignment[id] && node in nodeAssignment[id].nodes &&
                (nodeAssignment[id].nodes.lenght + 1) >= device.memoryErrorNodes) continue;

            const intersection = nodeTags.filter(tag => device.capabilities.includes(tag));

            // ignore if there is no intersection or the intersection
            // does not contain all the tags from the node
            if (intersection.length === 0 || intersection.length < nodeTags.length) continue;

            // Nodes with less nodes assigned and more specific intersections have a better match index
            const matchIndex = (1 / (device.nodes.length + 1)) * 0.9 +
                (intersection.length / device.capabilities.length) * 0.1;
    ...
    async distributeFlow(nodes, changes) {
        ...
        console.log("[MINE] Flow distribution - Nodes:", nodes);

        if (changes && this.context.nodeAssignment) {
            ...
            for (const id in this.context.devices) {
                ...
                if (changedDevice) {
                    this.context.devices[id] = { ...this.context.devices[id], ...changedDevice };
                }
            }
        }
        ...
        Object.entries(nodes).forEach(([k, node], index) => {
            if (node.type === "mqtt-broker") return;
            ...
            if (deviceId) {
                ...
                this.context.devices[deviceId].nodes.push(node);

                // Save this configuration in the current configuration object
                if (nodeAssignment[deviceId] && !(node in nodeAssignment[deviceId].nodes)) {
                    nodeAssignment[deviceId].nodes.push(node);
                } else {
                ...
                if (nodeAssignment["node-red"] && !(node in nodeAssignment["node-red"].nodes)) {
                    nodeAssignment["node-red"].nodes.push(node);
                } else {
        ...
        const assignmentResult = await this.generateAndSend(this.context.devices);
        ...
        for (const id in this.context.devices) {
            this.context.devices[id].running = true;
        }
    ...
    async heartbeat() {
        ...
        if (Object.keys(changes).length > 0) {
            this.distributeFlow(this.activeNodes, changes)
        }
    }
     ...
     * [MINE]
     * 
     * Sets up an interval for the heartbeat pattern.
     * 
     */
    setupHeartbeat() {
        // only one heartbeat is running
     ...
     * @return {[type]}     [description]
     */
    start(diff, deviceChanges) {
        // console.log("[MINE] START FLOW - from /nodes/flows/index .start(): ", diff);
        ...
        // [MINE] 
        // Setup MQTT topics for each node to communicate with 
        // their respective input and output
        for (const id in this.flow.nodes) {
            node = this.flow.nodes[id];

            if ((node.wires.length === 0) || (node.wires.length === 1 && node.wires[0].length === 0)) continue;

            for (const wire of node.wires[0]) {
                const topicName = this.TYPE === "subflow" ? `topic${this.id.replace(/\./g, "")}_node_sub` : `topic${indexMQTT}_node`;
                indexMQTT++;
                // if node is wired to outside the subflow
                // there is no need to create another mqtt topic
                let toOutsideSubflow = false;
                if (this.TYPE === "subflow") {
                    const results = Object.keys(this.flow.nodes).filter(id => id === wire);
                    if (results.length === 0) {
                        toOutsideSubflow = true;
                        return;
                    }
                }

                // create another topic if the node is wired to
                // a node inside the same flow
                if ((this.TYPE === "subflow" && !toOutsideSubflow) || this.TYPE !== "subflow") {
                    node.outputTopics.push(topicName);
                }

                if (this.flow.nodes[wire]) {
                    this.flow.nodes[wire].inputTopics.push(topicName);
                }
            }
        }
        ...
        // instanciate as Node all the nodes inside the subflow
        for (const subflowId in this.subflowInstanceNodes) {
            const subflow = this.subflowInstanceNodes[subflowId]
            for (const id in subflow.node_map) {
                const newNode = flowUtil.createNode(this, { ...subflow.node_map[id] });
                this.activeNodes[newNode.id] = newNode;
            }
        }

        // distributes the flow through the available devices
        this.distributeFlow(this.activeNodes, deviceChanges);
