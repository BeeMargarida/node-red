...
def exec(mqtt_c):
     ...
     */
    sendCode(code, device) {
        axios({
            method: 'POST',
            url: `http://${device}/execute`,
            data: code,
            headers: {
    ...
    start(diff) {
        ...
        const devices = ['192.168.1.81', '192.168.1.193'];
        let i_dev = 0;
                ...
                if (node.d !== true) {
                    if (!node.subflow) {
                        if (!this.activeNodes[id]) {
                            ...
                            const device = i_dev >= devices.length ? null : devices[i_dev];
                            const local = device === null ? true : false;

                            newNode = flowUtil.createNode(this, { ...node, local: true, device: device });
        ...
        for(const subflowId in this.subflowInstanceNodes) {
            ...
            for(const id in subflow.node_map) {
                ...
                this.activeNodes[newNode._alias] = newNode;
            }
        }

        // const chosenNodes = Object.values(this.activeNodes).slice(0, 2);
        // const code = this.generateCode(chosenNodes);
        
        // // @TODO: Remove later
        // const fs = require('fs');
        // fs.writeFile(`python-scripts/total-code.py`, code, function (err) {
        //     if (err) {
