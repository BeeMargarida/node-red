...
def exec(mqtt_c):
    ...
    distributeFlow(nodes) {
        // @TODO: get this information from the network
        const devices = [
            { address:'192.168.1.81', status: 1, running: false, state: {}, ram: "100", cpu: "50", capabilities: ["temperature", "humidity"] }, 
            { address: '192.168.1.193', status: 1, running: false, state: {}, ram: "100", cpu: "80", capabilities: [] }
        ];
        let i_dev = 0;
        ...
        devices.forEach(device => {
            const deviceNodes = Object.entries(nodes).filter(([k, node]) => (node.device === device));
            const code = this.generateCode(deviceNodes);
            this.sendCode(code, device);

            // @TODO: Remove later
            const fs = require('fs');
            fs.writeFile(`python-scripts/total-code-${device.address}.py`, code, function (err) {
                if (err) {
    ...
    start(diff) {
        ...
        this.distributeFlow(this.activeNodes)
