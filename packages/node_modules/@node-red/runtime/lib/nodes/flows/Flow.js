...
class Flow {
    ...
    generateCode(nodes) {
        ...
        for (let i = 0; i < nodes.length; i++) {
            inputTopics.concat(nodes[i].inputTopics);
            outputTopics.concat(nodes[i].outputTopics);
        }
        ...
        // Remove not needed topics if more than one node
        const inputTopicsTmp = inputTopics.filter((el) => !outputTopics.includes(el));
        const outputTopicsTmp = outputTopics.filter((el) => !inputTopics.includes(el));

        inputTopics = inputTopicsTmp;
        outputTopics = outputTopicsTmp;

        // Global variables
        let code = `mqtt_client = None
input_topics = [${inputTopics.map(a => `"${a}"`)}]
output_topics = [${outputTopics.map(a => `"${a}"`)}]`;

        // @TODO - MQTT input handling and specific node code

        // General code
        code += `def on_output(msg):
...
def cancel():
    ...
    start(diff) {
        ...
        for (id in this.flow.nodes) {
            if (this.flow.nodes.hasOwnProperty(id)) {
                ...
                if (node.d !== true) {
                    if (!node.subflow) {
                        if (!this.activeNodes[id]) {
                            console.log("[MINE] Create node: ", node.wires);

                            const device = i_dev >= devices.length ? null : devices[i_dev];
                            const local = device === null ? true : false;

                            newNode = flowUtil.createNode(this, { ...node, local: local, device: device });
                            ...
                            if (newNode) {
                                ...
                                if (!newNode.local) this.sendCode(newNode.micropythonCode, newNode.device);
        ...
        // console.log("[MINE] Active Nodes: ", this.activeNodes);
