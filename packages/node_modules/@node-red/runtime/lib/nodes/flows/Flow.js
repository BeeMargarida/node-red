...
class Flow {
    ...
    start(diff) {
        ...
        for(const id in this.flow.nodes) {
            ...
            node.inputTopics = node.inputTopics ? node.inputTopics : [];
            node.outputTopics = node.outputTopics ? node.outputTopics : [];
            
            for(const wire in node.wires) {
                node.outputTopics.push(`mqtt/topic${indexMQTT}`);
                this.flow.nodes[wire].inputTopics.push(`mqtt/topic${indexMQTT}`);
                indexMQTT++;
            }
        }
