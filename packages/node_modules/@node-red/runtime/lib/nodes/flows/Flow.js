...
async def exec(mqtt_c):
    ...
    getBestDevice(devices, node, nodeAssignment) {
        ...
        for (const id in devices) {
            ...
            if (predicateIntersection.length === 0 || predicateIntersection.length < nodePredicates.length) continue;
            ...
            const prioritiesIntersection = nodePriorities.filter(tag => device.capabilities.include(tag));
    ...
    async distributeFlow(nodes, changes) {
        ...
        if (Object.keys(this.context.nodeAssignment).length > 0) {
            const fs = require('fs');
            fs.writeFile(`python-scripts/node-assignment-${Date.now()}.json`,
                JSON.stringify(this.context.nodeAssignment),
                function (err) {
                    if (err) {
                        return console.log(err);
                    }
