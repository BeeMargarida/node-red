...
class Flow {
    ...
    generateCode(nodes) {
        ...
        // Global variables
        let code = `import uasyncio as asyncio
mqtt_client = None
nodes_id = [${nodesId}]
...
def exec(mqtt_c):
    ...
    for id in nodes_id:
        if hasattr("exec_" + id):
            func = getattr(self,"exec_" + id, None) 
            func()
    return`
    ...
    getBestDevice(devices, node) {
        ...
        for (const id in devices) {
            ...
            // Nodes with more specific intersections and less nodes assigned have a better match index
            const matchIndex = (intersection.length / device.capabilities.length) + 1/(device.nodes.length + 1);
    ...
    distributeFlow(nodes, changes) {
        ...
        const nodeAssignment = this.context.nodeAssignment ? this.context.nodeAssignment : {};

        // @TODO: Check changes of devices and make changes according to them
        ...
        Object.entries(nodes).forEach(([k, node], index) => {
            ...
            const deviceId = this.getBestDevice(devices, node);

            if (deviceId) {
                node.setDevice(devices[deviceId]);
                devices[deviceId].nodes.push(node);
        ...
        for(const id in devices) {
            if(devices[id].nodes.length === 0) return;

            const code = this.generateCode(devices[id].nodes);
            this.sendCode(code, id);
    ...
    async heartbeat() {
        ...
        for (const device of devices) {
            axios({
                method: 'GET',
                url: `http://${device.address}/ping`
            }).then(res => {
                console.log(`DEVICE ${device.address} STATUS: ` + res);
                // @TODO: Update context nodeAssignment with change
            })
                .catch(err => {
                    console.log(`DEVICE ${device.address} ERROR: ` + err);
                    // @TODO: Update context nodeAssignment with change
                })
        }
    ...
    start(diff, deviceChanges) {
        ...
        // console.log("[MINE] START FLOW: Config Nodes: ", this.flow.configs);
        ...
        // console.log("[MINE] Flow nodes:", this.flow.nodes);
