...
async def exec(mqtt_c):
    ...
    sendCode(code, address) {
        return axios({
            ...
            console.log(err);
            return { id: address, status: err.response ? err.response.status : 500 }
        })
    }
    ...
    async generateAndSend(devices) {
        ...
        for (const id in devices) {
            ...
            promises.push(this.sendCode(code, id));
    ...
    getBestDevice(devices, node, nodeAssignment) {
        ...
        for (const id in devices) {
            ...
            // if node was already assigned to the device 
            // and the device can handle it, keep it
            if (nodeAssignment[id] && node in nodeAssignment[id].nodes && !device.memoryError) return id;

            if (nodeAssignment[id] && node in nodeAssignment[id].nodes && device.memoryError) {
                // ignores device and removes memory error state,
                // since the device will not have this node assigned to it
                device.memoryError = false;
                continue;
            }
            ...
            if (matchIndex > bestMatchIndex && device.ram >= 50 && device.cpu >= 50) {
    ...
    async heartbeat() {
        ...
        for (const id in this.context.devices) {
            axios({
                ...
                console.log(`DEVICE ${id} STATUS: ` + JSON.stringify(res));

                // checks if status in the node changed
                if (this.context.nodeAssignment[id].status !== 1) {
                    this.context.nodeAssignment[id].status = 1;
                    this.changes[id] = { status: 1 }
                }
            })
                .catch(err => {
                    ...
                    // checks if status in the node changed
                    if (this.context.nodeAssignment[id].status !== 0) {
                        this.context.nodeAssignment[id].status = 0;
                        this.changes[id] = { status: 0 }
                    }
                })
        }
    ...
    start(diff, deviceChanges) {
        ...
        for (const id in this.flow.nodes) {
            ...
            for (const wire of node.wires[0]) {
                node.outputTopics.push(`topic${indexMQTT}_node`);
                if (this.flow.nodes[wire]) {
                    this.flow.nodes[wire].inputTopics.push(`topic${indexMQTT}_node`);
                }
                indexMQTT++;
            }
        }
