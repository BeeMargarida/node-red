...
class Subflow extends Flow {
        ...
        if (subflowDef.nodes) {
            ...
            for (i in subflowDef.nodes) {
                if (subflowDef.nodes.hasOwnProperty(i)) {
                    ...
                    if ((node.wires.length !== 0) && (node.wires.length !== 1 && node.wires[0].length !== 0)) {
                        for (const wire of node.wires[0]) {
                            node.outputTopics.push(`mqtt/topic_sub${indexMQTT}`);
                            subflowDef.nodes[wire].inputTopics.push(`mqtt/topic_sub${indexMQTT}`);
                            indexMQTT++;
                        }
                    }
    ...
    start(diff) {
        ...
        if (this.subflowDef.in) {
            ...
            // [MINE] Connect subflow input into first nodes input
            for(const wire of subflowInstanceConfig.wires) {
                if(this.node_map[wire]) {
                    this.node_map[wire].inputTopics = this.subflowInstance.inputTopics;
                }
            }
        }
            ...
            for (var i = 0; i < this.subflowDef.out.length; i++) {
                ...
                for (var j = 0; j < wires.length; j++) {
                    if (wires[j].id === this.subflowDef.id) {
                        ...
                        // TODO: [MINE] Handle this situation
                    } else {
                        var node = self.node_map[wires[j].id];

                        // [MINE] Connect subflow output into last node output
                        node.outputTopics = this.subflowInstance.outputTopics;
