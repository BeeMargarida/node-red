...
class Subflow extends Flow {
    ...
    constructor(parent, globalFlow, subflowDef, subflowInstance) {
        ...
        // for(const inEntry in subflowDef.in) {
        //     const wires = subflowDef.in[inEntry].wires;
        //     for (const wire of wires) {
        //         const [[key, _]] = Object.entries(subflowInternalFlowConfig.nodes).filter(([key,n]) => n._alias === wire.id);
        //         subflowInternalFlowConfig.nodes[key].inputTopics.push(...subflowInstance.inputTopics);
        //     }
        // }

        // for(const outEntry in subflowDef.out) {
        //     const wires = subflowDef.out[outEntry].wires;
        //     for (const wire of wires) {
        //         const [[key, _]] = Object.entries(subflowInternalFlowConfig.nodes).filter(([key,n]) => n._alias === wire.id);
        //         subflowInternalFlowConfig.nodes[key].outputTopics.push(...subflowInstance.outputTopics);
        //     }
        // }
    ...
    start(diff) {
        ...
        var subflowInstanceConfig = {
            ...
            wires: [],
            inputTopics: this.subflowInstance.inputTopics,
            outputTopics: this.subflowInstance.outputTopics,
            _flow: this
        }
        ...
        if (this.subflowDef.in) {
            for (const inEntry in this.subflowDef.in) {
                const wires = this.subflowDef.in[inEntry].wires;
                for (const wire of wires) {
                    const node = self.node_map[wire.id];
                    node.inputTopics.push(...this.node.inputTopics);
                }
            }
        }
            ...
            for (var i = 0; i < this.subflowDef.out.length; i++) {
                ...
                for (var j = 0; j < wires.length; j++) {
                    if (wires[j].id === this.subflowDef.id) {
                        ...
                        node.outputTopics.push(...this.node.outputTopics);
                    }
                }
            }
        }
