...
const configMQTT = require("./config/config");
...
function Node(n) {
    ...
    this.brokerUrl = configMQTT.brokerUrl;
    this.connectingMQTT = false;
    this.connectedMQTT = false;
    this.clientMQTT = null;

    this.inputTopics = n.inputTopics;
    this.outputTopics = n.outputTopics;
    this.subscriptionsMQTT = {};
    this.micropythonCode = "";
    this.hasInput = true;
    this.noWiresType = false;
    this.local = true;
    this.device = null;

    // Used to assign nodes to devices
    // predicates can't be violated, priorities may be violated 
    // if there is no way to implement them
    this.predicates = [];
    this.priorities = [];
...
Node.prototype.info = function () {
    const deviceInfo = Object.assign({}, this.device);
    if (deviceInfo) {
        delete deviceInfo.nodes
    }
    return {
        id: this.id,
        textId: this.textId,
        ...
        outputTopics: this.outputTopics,
        subscriptionsMQTT: this.subscriptionsMQTT,
        micropythonCode: this.micropythonCode,
...
Node.prototype.setupMQTT = function () {
    if(this.connectedMQTT) this.onCloseMQTT();
    this.brokerUrl = configMQTT.brokerUrl;
    this.connectingMQTT = true;
    this.connectedMQTT = false;

    this.options = {
        clientId: this.clientid || 'mqtt_' + (1 + Math.random() * 4294967295).toString(16),
        keepalive: 60,
        clean: true,
        reconnectPeriod: 150000,
        rejectUnauthorized: false
    }

    try {
        if (this.clientMQTT) this.clientMQTT.end(false);
        this.clientMQTT = mqtt.connect(configMQTT.brokerUrl, this.options);
        this.clientMQTT.setMaxListeners(0);

        // Register successful connect or reconnect handler
        this.clientMQTT.on('connect', () => this.onConnectMQTT());

        // Register disconnect handlers
        this.clientMQTT.on('close', () => this.onCloseMQTT());

        // Register connect error handler
        // The client's own reconnect logic will take care of errors
        this.clientMQTT.on('error', function (error) { });
    } catch (err) {
        console.log(err);
    }
}

Node.prototype.removeMQTT = function () {
    this.brokerUrl = null;
    this.connectingMQTT = false;
    this.connectedMQTT = false;
    this.clientMQTT.end();
...
Node.prototype.onConnectMQTT = function () {
    this.connectingMQTT = false;
    this.connectedMQTT = true;

    // Remove any existing listeners before resubscribing to avoid duplicates in the event of a re-connection
    this.clientMQTT.removeAllListeners('message');

    // Re-subscribe to stored topics
    for (const topic of this.inputTopics) {
        const qos = 2;
        this.subscribeMQTT(topic, { qos: qos });
    }
}

Node.prototype.onCloseMQTT = function () {
    if (this.connectedMQTT) this.connectedMQTT = false;
    this.clientMQTT.end();
}

Node.prototype.subscribeMQTT = function (topic, qos) {
    const self = this;
    const subscription = {
        topic: topic,
        qos: qos,
        handler: function (mtopic, mpayload, mpacket) {
            if (!redUtil.matchTopic(topic, mtopic)) return;
            self.onReceiveMQTT(mtopic, mpayload, mpacket)
        }
    };

    this.subscriptionsMQTT[topic] = subscription;

    if (this.connectedMQTT) {
        this.clientMQTT.on('message', subscription.handler);
        this.clientMQTT.subscribe(topic, qos);
    }

}

Node.prototype.unsubscribeMQTT = function (topic) {
    const subscription = this.subscriptionsMQTT[topic];

    if (!subscription) return;

    this.clientMQTT.removeListener('message', subscription.handler);
    delete this.subscriptionsMQTT[topic];

    if (this.connectedMQTT) this.clientMQTT.unsubscribe(topic);
}

Node.prototype.onReceiveMQTT = function (topic, payload) {
    var node = this;
    ...
    if (node.datatype === "buffer") {
        ...
        // Converts to string and converts to JSON if possible
    ...
    if (!payload._msgid) {
        payload._msgid = redUtil.generateId();
    }

    if (this._asyncDelivery) {
        setImmediate(function () {
            node._emitInput.apply(node, [{ topic: topic, payload: payload }]);
        });
    } else {
        this._emitInput.apply(this, { topic: topic, payload: payload });
    }
}

Node.prototype.publishMQTT = function (msg) {
    if (!this.connectedMQTT) return;
    ...
    for (const topic of this.outputTopics) {
        this.clientMQTT.publish(topic, msg.payload, options);
    }
}

Node.prototype.setMicropythonCode = function (code) {
    this.micropythonCode = code;
}

Node.prototype.setGenerateCodeFunction = function (func) {
    this.generateCode = func;
}

Node.prototype.setDevice = function (device) {
    this.device = device;
    this.local = device !== null ? false : true;
}

Node.prototype.setPredicates = function (predicates) {
    this.predicates = predicates;
}

Node.prototype.setPriorities = function (priorities) {
    this.priorities = priorities;
}

Node.prototype.setInput = function (value) {
    this.hasInput = value;
}

Node.prototype.setNoWiresType = function (value) {
    this.noWiresType = value;
}
...
Node.prototype.send = function (msg) {
    ...
    if (!sentMessageId) {
        sentMessageId = msg._msgid;
    }

    this.publishMQTT(msg);

    this.metric("send", { _msgid: sentMessageId });

    // TODO: Support two methods: events and mqtt
